...
// Converts text of typescript file to JSON of modules, class, functions and type with associated comments
function tsToJson(inp) {
	...
	let extractRecurse = (data, currentModule, currentClass) => {
		...
		if (dataHead.type === 'module') {
			...
			let pubs =
				doc[currentModule].c[dataHead.name].m['Class Constructor'].snip
				.match(/public.*/g);
			if (pubs) {
				pubs.forEach(p => {
					let name = p.replace('public', '').match(/[a-z0-9_$]+/i)[0];
					if (!doc[currentModule].c[dataHead.name].v) {
						doc[currentModule].c[dataHead.name]['v'] = {};
					}
					doc[currentModule].c[dataHead.name].v[name] = {
						desc: '',
						snip: p.replace(/[,;]$/, '')
					}
				});
	...
	let extractFuncDeclaration = inp => {
		let bracketCount = 0;
